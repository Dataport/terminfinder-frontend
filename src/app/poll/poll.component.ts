...
export class PollComponent implements OnInit {
  ...
  public sendReply(): void {
    ...
    this.apiError = null;
    const apiParticipantsToDelete: ApiParticipant[] = ModelTransformerService.transformParticipantsToApiParticipants(
      this.formHelper.participantsToDelete
    );
    this.logger.debug(
      `Übermittle Daten zum Löschen der Teilnehmer: ${JSON.stringify(apiParticipantsToDelete)}`,
      apiParticipantsToDelete
    );
    ...
    if (!NullableUtils.isObjectNullOrUndefined(updatedOrCreatedParticipants)) {
      ...
      this.logger.debug(
        `Übermittle Daten zum Erstellen/Aktualisieren der Teilnehmer: ${JSON.stringify(apiParticipants)}`,
        apiParticipants
      );

      this.dataRepoService
        .createOrUpdateVotingsAndDeleteParticipantsById(
        ...
        .then((result: Participant[]) => {
          this.logger.debug(
            `Daten der Teilnehmer erfolgreich versendet - Rückgabe von der API ist: ${JSON.stringify(result)}`,
            result
          );
      ...
      this.dataRepoService
        .deleteParticipantsOfAppointmentById(this.model.appointmentId, apiParticipantsToDelete)
        .then(() => {
  ...
  private initModel(): void {
    ...
    this.formHelper.participantsToDelete = [];
    this.dataRepoService
      .readAppointmentById(this.appointmentId)
      .then((data: ApiAppointment) => {
        ...
        if (
          !NullableUtils.isObjectNullOrUndefined(this.formHelper.appointment.participants) &&
          ...
          if (NullableUtils.isObjectNullOrUndefined(selectedParticipant)) {
            ...
            if (NullableUtils.isObjectNullOrUndefined(selectedParticipant.participantId)) {
              ...
              selectedParticipant = this.formHelper.appointment.participants.find(
                (value) => value.name === selectedParticipant.name
              );
        ...
        if (this.isAppointmentPaused) {
          this.formHelper.getTosFormControl().disable();
        } else {
          this.formHelper.getTosFormControl().enable();
        }
