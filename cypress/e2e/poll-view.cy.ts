...
context('poll-view', () => {
  ...
  describe('Poll options', () => {
    ...
    it('Options clickable', () => {
      ...
      cy.get('#desktop-voting-status-declined-0').should('not.be.checked');
      cy.get('#desktop-voting-status-questionable-0').should('not.be.checked');

      cy.get('#desktop-voting-status-declined-1').click();
      cy.get('#desktop-voting-status-questionable-1').click();
      cy.get('#desktop-voting-status-accepted-1').click();

      cy.get('#desktop-voting-status-questionable-1').click();
      cy.get('#desktop-voting-status-questionable-1').should('be.checked');
      cy.get('#desktop-voting-status-declined-1').should('not.be.checked');
      cy.get('#desktop-voting-status-accepted-1').should('not.be.checked');
    });
    ...
    it('Options change column counter', () => {
      ...
      cy.get('#desktop-voting-status-accepted-0').click();
      cy.get('#desktop-voting-status-accepted-0').should('be.checked');
      cy.get('#summary-column-0').should('contain.html', '1');
      cy.get('#summary-column-1').should('contain.html', '0');
      cy.get('#desktop-voting-status-questionable-0').click();
      ...
      cy.get('#summary-column-0').should('contain.html', '0');
      cy.get('#desktop-voting-status-accepted-1').click();
      cy.get('#summary-column-1').should('contain.html', '1');
      cy.get('#summary-column-0').should('contain.html', '0');
      cy.get('#desktop-voting-status-declined-1').click();
      cy.get('#summary-column-1').should('contain.html', '0');
    });
  ...
  describe('Edit participation', () => {
    ...
    it('Edit participation', () => {
      ...
      cy.wait('@apiCheckPutParticipant').then((interception) => {
        ...
        assert.equal(
          JSON.stringify(interception.request.body[0].votings),
          JSON.stringify([
            {
  ...
  describe('Download information', () => {
    ...
    it('Download ics files', () => {
      cy.get('#download-cal-0').click();
      const downloadedFilename1 = path.join(DOWNLOADS_FOLDER, 'Umfrage-Test-Titel-1.ics');
      cy.readFile(downloadedFilename1, 'binary', { timeout: 15000 }).should((buffer) =>
        expect(buffer.length).to.be.gt(100)
      );

      cy.get('#download-cal-1').click();
      const downloadedFilename2 = path.join(DOWNLOADS_FOLDER, 'Umfrage-Test-Titel-2.ics');
      cy.readFile(downloadedFilename2, 'binary', { timeout: 15000 }).should((buffer) =>
        expect(buffer.length).to.be.gt(100)
      );
    });
  });
